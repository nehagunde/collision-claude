...
| Collision-detection metric | **Time-To-Collision (TTC)** = distance ÷ relative closing speed, not a raw speed threshold. Threshold varies by **both** vehicle type (trucks get a longer safe TTC than cars, reflecting braking distance) **and road classification/segment type** (added 2026-08-17, per Paper 2's finding that unclassified/narrow roads and double-carriageway segments carry higher risk — segments with fewer lanes or lower classification in the real NH16 network from Phase 1 use a stricter/longer threshold, not a uniform one across the whole highway). |
